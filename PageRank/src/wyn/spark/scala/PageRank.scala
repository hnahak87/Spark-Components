...
object PageRank {
  ...
  def main(args: Array[String]) {
     if (args.length != 4 )
    {
      println("Need arguments: <app-name> <PhoneOwners-input-file> <PhoneCalls-input-file> <no. of iteration>")
      System.exit(-1)
    }
    ...
    val iters = args(3).toInt   // 1 is special case to handle phonecalls data which is generated randomly for 200mb/500mb files 
    val ctx = new SparkContext(sparkConf)
    val cedFile = ctx.textFile(args(2), 1)
    cedFile.persist(StorageLevel.MEMORY_AND_DISK)
    val calls = cedFile.map { s =>
      val parts = s.split(",") //s.split("\\s+") 
      (parts(0), parts(1))
